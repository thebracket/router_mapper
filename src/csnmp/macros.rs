...
macro_rules! make_oid {
    () => {
        $crate::csnmp::oid::ObjectIdentifier::new(0, [0u32; $crate::csnmp::oid::MAX_SUB_IDENTIFIER_COUNT])
    };
    ($firstnum:literal $(, $nextnums:literal)*) => {
        {
            let mut arcs = [0u32; $crate::csnmp::oid::MAX_SUB_IDENTIFIER_COUNT];
            let mut length = 0;
            make_oid!(@store_numbers, arcs, length, $firstnum $(, $nextnums)*);
            $crate::csnmp::oid::ObjectIdentifier::new(length, arcs)
        }
    };
